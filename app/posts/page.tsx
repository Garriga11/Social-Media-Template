...
export default async function Home() {
    ...
    function PostInputs() {
        return (
            <div>
                ...
                {posts.map((post: { id: number; content: string | null; createdAt: Date; author: { firstName: string; lastName: string } }) => (
                    <div key={post.id} className="p-4 border border-zinc-800 rounded mt-4">
                        <h2 className="font-bold">{`${post.author.firstName} ${post.author.lastName}`.trim() || "Unknown Author"}</h2>
    ...
    return (
        <main className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4">
                        <h2 className="font-bold">{posts.length > 0 ? `${posts[0].author.firstName} ${posts[0].author.lastName}`.trim() || "Unknown Author" : "No Posts Available"}</h2>
            <PostInputs/>
            <div className="mt-8">
                {posts.map((post: { id: number; content: string | null; createdAt: Date; author: { firstName: string; lastName: string } }) => (
                    <div
